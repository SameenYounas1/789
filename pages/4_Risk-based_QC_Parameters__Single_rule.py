...
st.set_page_config(layout="wide", page_title="QC Leap", page_icon=":dna:")
...
st.markdown("### **:blue[Risk-based QC Parameters: Single Rule Scheme]**")
st.write(" ")
...
Ped_for_QC_rule_P1 = [1 - ((norm.cdf(QC_rules_1ks - x) - norm.cdf(-QC_rules_1ks - x))**number_of_QCs) for x in occurance_of_error_as_SDs]
...
if probability_of_false_rejection_Pfr_for_QC_rules > 0.05 and number_of_QCs > 1:
...
elif probability_of_false_rejection_Pfr_for_QC_rules > 0.05 and number_of_QCs == 1:
...
#occurance_of_SE_input_value  = st.number_input('**:blue[Systematic error (%) of concern]**', min_value=0.0, max_value=33.0, step = 0.01)

# Occurrence of systematic error (s) as multiples of CV
occurance_of_SE_in_imp_value = SE_of_concern

# Calculate the Probability of error detection for QC rules after an error occurs (P1)
Ped_P1_value = 1 - ((norm.cdf(QC_rules_1ks - occurance_of_SE_in_imp_value) - norm.cdf(-QC_rules_1ks - occurance_of_SE_in_imp_value)) ** number_of_QCs)

# expected number of QC events to detect a systematic error E(QCE) AKA ARLed
e_QCE = 1/Ped_P1_value
...
# Probability of exceeding the TEa fraction when error exists
p_TEa_exceed_out_control = norm.cdf(-sigma_value_without_bias - bias_as_multiples_of_CV - occurance_of_SE_in_imp_value) + (1 - norm.cdf(sigma_value_without_bias - bias_as_multiples_of_CV - occurance_of_SE_in_imp_value))
...
for se in SE_list:
    ped_result = 1 - ((norm.cdf(QC_rules_1ks - se) - norm.cdf(-QC_rules_1ks - se)) ** number_of_QCs)
    ped_list.append(ped_result)


# POWER CURVE
# Prepare the higher X-axis data
# Prepare the higher X-axis data
sigma_values = [x + 1.65 for x in occurance_of_error_as_SDs]
...
fig223.add_trace(
    go.Scatter(x=occurance_of_error_as_SDs, y=Ped_for_QC_rule_P1, name="SE values", mode='lines+markers')
)

# Generate tick values for the x-axis at 0.5 intervals
max_x_value = max(occurance_of_error_as_SDs)
